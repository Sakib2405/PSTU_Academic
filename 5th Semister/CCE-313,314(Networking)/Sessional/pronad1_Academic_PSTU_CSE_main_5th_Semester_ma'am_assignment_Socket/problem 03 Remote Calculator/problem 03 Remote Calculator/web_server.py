...
app = Flask(__name__)
# Use environment variables for secrets in production; provide a clear fallback for development.
app.secret_key = os.environ.get('REMOTE_CALC_SECRET', 'replace-this-with-a-secure-random-key')
app.config['SHUTDOWN_TOKEN'] = os.environ.get('REMOTE_CALC_SHUTDOWN_TOKEN', 'please-change-this-token')
...
def calculate(num1, operator, num2):
    ...
    Accepts integer or float inputs.
    """
    ...
    try:
        # Accept floats and ints; parse with float()
        a = float(num1)
        b = float(num2)
    ...
    # If the result is mathematically an integer, return as int for cleaner output
