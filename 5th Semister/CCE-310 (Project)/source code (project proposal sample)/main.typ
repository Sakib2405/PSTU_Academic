...
- @pstu_old_diary_admin *PSTU Diary (পবিপ্রবি ডায়েরি) - Admin Version* - Only for Admin, with unstable API. It is not available for general users and has limited functionality. Database is not well-structured. And it uses MySQL as for the database source, which is not well protected with row level security @pstu_old_diary_admin
...
#figure(
  grid(
    columns: (auto, auto),
    rows: (auto, auto),
    [ #image("UI/4.png", width: 61%) ], [ #image("UI/1.png", width: 61%) ],
  ),
